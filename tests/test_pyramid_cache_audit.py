...
def fixture():
    arms={'control':('control',False),'pyramid':('control',True)}
    ...
    for mask in range(8):
        ...
        for arm in arms:
            fresh=arm=='pyramid';name=f'{arm}_{mask}'
            c=deepcopy(case)
            if fresh:c['config']['pyramid_enabled']=True
...
def test_cache_or_trial_misrepresentation_is_rejected(change):
    ...
    elif change=='fake_reuse':report['cases']['pyramid_0']['cache_hit']=True
