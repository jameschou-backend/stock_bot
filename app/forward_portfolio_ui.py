...
def render():
    ...
    st.caption('新版獨立帳本｜按上一收盤總資產複利配置｜不送券商、不用報價假設成交')
    try: data=p.summary()
    ...
    with st.expander('今日結算與下一交易日計畫',expanded=True):
        reviewed=st.checkbox('已核對持股的除息、分割及股款交付；如有事件已先登錄',key='pf_actions_reviewed')
        left,right=st.columns(2)
        if left.button('封存今日收盤資產',key='pf_close',use_container_width=True):
            try:
                service.capture_close(actions_reviewed=reviewed)
                st.success('已封存；再次操作保留第一次紀錄。');st.rerun()
            except (ValueError,KeyError) as exc: st.error(str(exc))
        if right.button('建立下一交易日紙上計畫',key='pf_plan',use_container_width=True):
            try:
                plans=service.save_proposals()
                st.success(f'已保存 {len(plans)} 筆計畫／資金需求。');st.rerun()
            except (ValueError,KeyError) as exc: st.error(str(exc))
        st.caption('個股最多3檔，每檔上限為前日總資產的1/3；剩餘資金提出0050計畫。需要賣0050才能買個股時，先保留資金需求，等賣出成交才釋放買單。')
    if data['holdings']:
    ...
    if data['orders']:
        ...
        with st.expander('登錄部分／全部成交，或取消剩餘委託'):
            order=st.selectbox('紙上委託',data['orders'],format_func=lambda o:f"{o['order_id']}｜{o['status']}",key='pf_order')
            with st.form('pf_fill_form'):
            ...
            if saved:
                try:
                    ...
                    with j.connection(p.PATH) as con: p.submit(con,c)
                    st.success('成交已入帳，持股與剩餘預留已更新。');st.rerun()
                except (ValueError,KeyError) as exc: st.error(str(exc))
            reason=st.text_input('取消剩餘原因',value='核對當日回報，確認剩餘未成交',key='pf_cancel_reason')
            if st.button('取消剩餘數量',key='pf_cancel'):
                try:
                    with j.connection(p.PATH) as con:
                        p.submit(con,dict(kind='cancel',id=order['order_id'],order_id=order['order_id'],reason=reason))
                    st.rerun()
                except (ValueError,KeyError) as exc: st.error(str(exc))
    intents=[r for r in data['rows'] if r['kind']=='funding_intent']
    if intents:
        st.warning('曾有個股因現金不足而保留盤前資金需求；不代表已下單或已成交。')
        if st.button('0050成交入帳後，釋放今日資金需求',key='pf_release'):
            try:
                service.release_funded_intents();st.rerun()
            except (ValueError,KeyError) as exc: st.error(str(exc))
    with st.expander('規則、公司行動與完整帳本'):
        st.write('出場：收盤跌至調整後進場價的88%，或持有63個交易日，下一交易日提出限價賣出；未成交剩餘持股保留出場決策。限價委託不保證成交。')
        st.write('股息先列應收，實際入帳後才可使用；分割股未交付前禁止交易及完整估值。減資、碎股等未支援事件須暫停並對帳。')
        st.write('零股即時深度尚未接入；0050同條件前向比較帳本與自動公司行動核對尚未完成。此頁不顯示策略超額報酬。')
        report=st.file_uploader('進階：已核對公司行動 JSON（entitlement／delivery）',type=['json'],key='pf_action')
        if st.button('登錄公司行動證據',disabled=report is None,key='pf_action_save'):
            try:
        ...
        st.download_button('下載新版完整帳本',json.dumps(data['rows'],ensure_ascii=False,indent=2),'portfolio-v2.json','application/json',key='pf_download')
