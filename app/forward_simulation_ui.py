...
def render():
    ...
    if not sim.ROOT.exists():
        st.info('尚未初始化；必須在初始計畫交易日開盤前建立。');return
    try:
        books={role:p.summary(sim.ROOT/(role+'.sqlite3')) for role in ('strategy','benchmark')}
        for role in books:
            with sim.j.connection(sim.ROOT/(role+'.sqlite3')) as con:sim.verify(con)
    ...
    with st.expander('固定撮合規則與自動流程'):
        ...
        st.json(sim.RULES)
    report=sim.ROOT/'latest-report.json'
    if report.exists():
    ...
    path=sim.ROOT/(role+'.sqlite3');account='模擬_'+role
    ...
    with st.expander('完成今日公司行動核對／登錄權益'):
        ...
        if st.button('保存今日核對',disabled=not checked,key='sim_approve_'+role):
            try:auto.approve(path,reviewer,note);st.success('核對已保存，下一次排程會嘗試完成結算。')
