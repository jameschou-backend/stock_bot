...
def run(config, db_session: Session, **kwargs) -> Dict:
    ...
    Returns:
        包含抓取統計的 dict
    """
    job_id = start_job(db_session, "ingest_margin_short")
    logs: Dict[str, object] = {}
    ...
    try:
        ...
        for chunk_start, chunk_end in date_chunks(start_date, end_date, chunk_days=config.chunk_days):
            chunk_count += 1
            chunk_logs: Dict[str, object] = {}
            
            df = _fetch_margin_smart(chunk_start, chunk_end, config.finmind_token, chunk_logs)
