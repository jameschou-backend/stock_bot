"""Cash-funded event baskets with already-lagged market-state commands.
...
def simulate_regime(close: pd.DataFrame, flags: pd.DataFrame, events: list[dict],
                    control: pd.DataFrame, *, policy='idle_cash',
                    start='2022-01-03', end='2026-06-23', slots=3, horizon=63,
                    slippage=.003) -> dict:
    """Park idle assets according to causal ON/OFF/UNKNOWN commands.
    ...
    A basket sells at most current NAV / slots of 0050; net proceeds fund equal
    member budgets including purchase costs. The caller filters original signal
    days, and the engine additionally requires an ON command on execution day.
    UNKNOWN keeps the last known parking target (initially cash), blocks entries,
    and never issues a new forced exit. OFF under exit_cash latches stock exits
    until the whole basket can trade, even if a later command returns to ON.

    At entry+horizon sessions a scheduled exit requires 0050 only when its idle
    allocation target is ON. Forced and terminal stock exits always go to cash,
    independently of 0050 tradability. Exits precede idle allocation and entries.
    Final blocked holdings stay explicitly marked; no liquidation is invented.
    """
    if policy not in {'idle_cash', 'exit_cash'}:
        raise ValueError('policy must be idle_cash or exit_cash')
    ...
    if (not isinstance(control, pd.DataFrame) or not control.index.equals(close.index)
            or not control.columns.is_unique or set(control.columns) != {'state', 'decision_date'}):
        raise ValueError('Control must align exactly with prices and contain state and decision_date')
    commands, decisions = [], []
    for execution_day, row in control.iterrows():
        state = row['state']
        if not isinstance(state, str) or state not in {'ON', 'OFF', 'UNKNOWN'}:
            raise ValueError('Control state must be ON, OFF, or UNKNOWN')
        missing_date = row['decision_date'] is None or pd.isna(row['decision_date'])
        if state != 'UNKNOWN' and missing_date:
            raise ValueError('Known commands require a decision_date strictly before execution')
        decision = None if missing_date else _date(row['decision_date'], 'decision_date')
        if decision is not None and decision >= execution_day:
            raise ValueError('decision_date must be strictly before its execution date')
        commands.append(state)
        decisions.append(None if decision is None else str(decision.date()))
    ...
    window_indices = close.index.get_indexer(days)
    commands = [commands[index] for index in window_indices]
    decisions = [decisions[index] for index in window_indices]
    ...
    for supplied in events:
        ...
        reason = ('outside_window' if entry < days[0] or entry > days[-1]
    ...
    state_changes, etf_blocked, forced_exit_requests = [], [], []
    park_state, park_decision_date, previous_command = 'CASH', None, None
    ...
    def close_cohort(i, cohort, terminal, forced):
        entry_nav = nav(i)
        member_results, exit_proceeds, sell_cost = [], 0., 0.
        reason = 'forced_exit' if forced else 'terminal_liquidation' if terminal else 'scheduled_exit'
        ...
        repurchased = (fill(i, 0, 'buy', exit_proceeds, cohort['event_id'], 'return_to_benchmark')
                       if not terminal and not forced and park_state == 'ON'
                       else {'cost': 0., 'units': 0.})
        ...
        completed.append({**{k: v for k, v in cohort.items() if k not in {'entry_index', 'columns'}},
                          ...
                          'forced_exit_executed': bool(forced),
                          'exit_park_state': park_state,
    ...
    for i, day in enumerate(days):
        ...
        state, decision_date = commands[i], decisions[i]
        previous_park = park_state
        if state in {'ON', 'OFF'}:
            park_state, park_decision_date = state, decision_date
        if state != previous_command:
            state_changes.append({'date': str(day.date()), 'from_state': previous_command,
                                  'to_state': state, 'decision_date': decision_date,
                                  'from_park_state': previous_park, 'park_state': park_state,
                                  'park_decision_date': park_decision_date})
        previous_command = state
        if policy == 'exit_cash' and state == 'OFF':
            for cohort in active:
                if not cohort['forced_exit_requested_on']:
                    cohort['forced_exit_requested_on'] = str(day.date())
                    cohort['forced_exit_decision_date'] = decision_date
                    forced_exit_requests.append({'event_id': cohort['event_id'],
                                                 'date': str(day.date()), 'decision_date': decision_date,
                                                 'members': cohort['members']})

        for cohort in list(active):
            forced = bool(cohort['forced_exit_requested_on'])
            if not terminal and not forced and i < cohort['entry_index'] + horizon:
                continue
            needs_benchmark = not terminal and not forced and park_state == 'ON'
            required = ([0] if needs_benchmark else []) + cohort['columns']
            if not bool(np.all(tradable[i, required])):
                ...
                cohort['blocked_exits'].append({'date': str(day.date()),
                                               ...
                                               'terminal': terminal, 'forced': forced,
                                               'park_state': park_state})
                if needs_benchmark and not tradable[i, 0]:
                    etf_blocked.append({'date': str(day.date()), 'action': 'scheduled_return_to_benchmark',
                                        'event_id': cohort['event_id'], 'state': state,
                                        'park_state': park_state, 'decision_date': decision_date,
                                        'park_decision_date': park_decision_date})
                continue
            close_cohort(i, cohort, terminal, forced)
        ...
        if terminal:
            if units[0] > 0:
                if tradable[i, 0]:
                    ...
                    etf_blocked.append({'date': str(day.date()), 'action': 'terminal_liquidation',
                                        'event_id': None, 'state': state, 'park_state': park_state,
                                        'decision_date': decision_date, 'park_decision_date': park_decision_date})
        # Keep round-off cash explicitly rather than generating a microscopic
        # ETF fill every day. This is not a minimum trade/lot-size assumption.
        elif park_state == 'ON' and cash > 32 * np.finfo(float).eps * nav(i):
            if tradable[i, 0]:
                fill(i, 0, 'buy', cash, None, 'initial_benchmark' if i == 0 else 'park_on')
            else:
                etf_blocked.append({'date': str(day.date()), 'action': 'buy_benchmark',
                                    'event_id': None, 'state': state, 'park_state': park_state,
                                    'cash_budget': float(cash), 'decision_date': decision_date,
                                    'park_decision_date': park_decision_date})
        elif park_state in {'OFF', 'CASH'} and units[0] > 0:
            if tradable[i, 0]:
                fill(i, 0, 'sell', units[0], None, 'park_off')
            else:
                etf_blocked.append({'date': str(day.date()), 'action': 'sell_benchmark',
                                    'event_id': None, 'state': state, 'park_state': park_state,
                                    'units': float(units[0]), 'decision_date': decision_date,
                                    'park_decision_date': park_decision_date})
        ...
        for event in scheduled.get(i, []):
            ...
            reason = ('terminal_session' if terminal else 'market_state_not_on' if state != 'ON'
                      else 'overlapping_member' if overlapping
            ...
            if reason:
                ...
                if reason == 'market_state_not_on':
                    rejected.update(state=state, decision_date=decision_date, park_state=park_state)
                if overlapping:
            ...
            cohort = {**event, 'entry_index': i, 'columns': columns, 'entry_nav': before,
                      ...
                      'forced_exit_requested_on': None, 'forced_exit_decision_date': None,
        ...
        curve.append({'date': str(day.date()), 'nav': equity, 'cash': float(cash),
                      'cash_weight': float(cash / equity), 'state': state, 'decision_date': decision_date,
                      'park_state': park_state, 'park_decision_date': park_decision_date,
    ...
    summary = {'total_return': float(navs[-1] - 1), 'cagr': float(navs[-1] ** (1 / years) - 1),
               ...
               'etf_blocked_sessions': len({item['date'] for item in etf_blocked}),
               'etf_blocked_actions': len(etf_blocked),
               'forced_exit_requested_cohorts': len(forced_exit_requests),
               'forced_exit_completed_cohorts': sum(bool(item['forced_exit_requested_on']) for item in completed),
               'forced_exit_pending_cohorts': sum(bool(item['forced_exit_requested_on']) for item in open_cohorts),
               ...
               'mode': policy, 'policy': policy, 'slots': int(slots), 'horizon': int(horizon),
               ...
               'allocation_policy': 'lagged ON parks cash in 0050 then funds baskets from net ETF sales; OFF parks idle assets in cash; UNKNOWN preserves the last known park target and blocks entries',
    ...
    return {'summary': summary, 'curve': curve, 'executions': executions,
            'state_changes': state_changes, 'etf_blocked': etf_blocked, 'forced_exit_requests': forced_exit_requests,
