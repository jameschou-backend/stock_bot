...
def run(config, db_session: Session, **kwargs) -> Dict:
    ...
    try:
        ...
        logger.info("[ingest_kbar_features] %s ~ %s", start_date, end_date)
        ...
        logger.info("kbar_features: %d 筆", total_rows)
        finish_job(db_session, job_id, "success", logs=logs)
        return {"rows": total_rows}

    except Exception as exc:
        logger.error("[ingest_kbar_features] 失敗: %s", exc, exc_info=True)
        try:
            db_session.rollback()
        except Exception as rb_exc:
            logger.warning("[ingest_kbar_features] rollback 失敗: %s", rb_exc)
        try:
            finish_job(db_session, job_id, "failed", error_text=str(exc), logs=logs)
        except Exception as finish_exc:
            logger.warning(
                "[ingest_kbar_features] finish_job 寫入失敗（保留原始例外）: %s",
                finish_exc,
            )
        raise
