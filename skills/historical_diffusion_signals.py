"""Versioned historical-eligibility diffusion engine.

Fork of the sealed skills.diffusion_signals builder. The only numerical changes
are dated eligibility masking, signal eligibility, and expected-cohort counts.
A neutral mask must reproduce the old entries; do not edit the sealed engine.
"""
...
from skills.diffusion_signals import (
    ARMS, LOOKBACK, MIN_COMMON, MIN_TURNOVER, _date, _validate, _rolling, _returns)
...
def build_diffusion(close: pd.DataFrame, other_close: pd.DataFrame,
                    ...
                    signal_end='2025-12-31', eligibility=None) -> dict:
    ...
    if eligibility is not None:
        if (not eligibility.index.equals(days)
                or set(eligibility.columns) != set(listed_dates) | {'0050'}
                or any(dtype != bool for dtype in eligibility.dtypes)):
            raise ValueError('Eligibility requires complete boolean calendar/cohort alignment')
        listed &= eligibility.reindex(columns=ids).to_numpy()
    ...
    quality &= benchmark_ok[:, None] & listed
    ...
    expected_count = (np.searchsorted(listing_times, days.to_numpy(), side='right')
                      if eligibility is None else eligibility.drop(columns='0050').sum(axis=1).to_numpy())
