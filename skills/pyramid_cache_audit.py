...
def audit_cache_reuse(report, parent, trials, arms, load_case):
    ...
    if set(report['cases']) != expected:
        raise ValueError('Addition comparison requires every registered arm and stress')
    ...
    for arm,(mode,enabled) in arms.items():
        for mask in range(8):
            ...
            if enabled:
                if row['reused_case'] is not None or case['config'].get('pyramid_enabled') is not True:
                    raise ValueError('A new addition account cannot claim a reused result')
                executed.add(name)
            else:
