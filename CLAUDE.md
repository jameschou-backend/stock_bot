...
**基準 v2.1（2026-07-10 benchmark 誠實化後，現行 headline）**：Sharpe **0.613**、MDD **-38.6%**、
累積 **+374%**、**超額 +80pp**（對零成本等權大盤 +294%）；歸因臂（無門檻）0.842 / +850% / 超額 +563pp。
v2→v2.1 變因：①benchmark 不再每月扣全額來回成本（原口徑虛增超額 ~190pp+）②market filter 改讀
零成本原始市場報酬（策略路徑微變）。JSON：`backtest_20260710_131824/132948.json`（zero_cost 口徑標記）。
⚠️ 誠實結論：生產口徑對等權大盤的超額僅 ~+8pp/年 → ~+1.5pp/年級別，微型股臂（+563pp）是主要價值來源。

**基準 v2（2026-07-03 全面健檢修復後，已被 v2.1 取代，數字供對照）**：Sharpe **0.65**、MDD **-37%**、
累積 **+431%**、超額 **+321%**（對含息+同流動性門檻大盤 +110%）、年化 **+18.5%**
（`--production-baseline`：topn 30 + 0.5 億流動性門檻 + 流動性加權 + SHAP 剪枝 58 特徵 + OHLC 全還原 + 含息 P&L）。
**歸因臂（同快照、無流動性門檻＝舊研究口徑）**：Sharpe **0.85**、累積 **+970%**、MDD **-43%**——
兩臂差 = 0.5 億門檻的真實代價（微型股 alpha；2023 年門檻臂 +6.7% vs 無門檻 +70.7%）。

> ⚠️ **舊 headline 全部退役**（+1508%/+5115%/Sharpe 1.06、以及 2026-06-24 的 0.99/+1321%）：
> 0.99 建立在**損壞特徵**上——adj_close 只乘在 close，open/high/low 未還原，ATR/KD/CCI/CMF/
> trend_persistence 在歷史段是物理不可能的值（2016 ATR% 中位數 42.8%）且形成日期 proxy；
> 另有 10.7 萬列 factor 內部缺日 fillna(1.0) 假跳動、生產 daily_pick 自 2/13 起 index 錯位
> （picks 用別檔股票的特徵打分）。2026-07-03 全部修復 + 全量重建 + 重訓（詳見
> `artifacts/review_pack_health_check_20260703.md` 與 memory/decisions.md 同日條目）。
> **基準 v2 的口徑變更**：首次把 `min_avg_turnover=0.5 億`（生產 universe 門檻）套進回測與 benchmark
> ——回測數字現在描述的就是實際部署的東西（含部署模型=回測模型：同參數/58 特徵/流動性加權/全窗重訓）。
> ⚠️ **post-sponsor 限制**：adj 只到 2026-06-23 快照；新交易日 factor 由 per-stock ffill 沿用最後值
> （2026-07-03 起，不再跳回 1.0），但 6/24 後的新除權息事件仍未調整；長期需 TWSE 除權息公告自算 factor。
> **殘餘偏差**：point-in-time（T 日成交盤後籌碼）、160 檔無 adj 下市股、14 檔 factor 觸 clip
> （`artifacts/adj_prices/adj_factor_clip_touched.json`）、DSR 多重檢定 → 0.65 仍偏樂觀端。
> 當前 **FEATURE_COLUMNS=87、PRUNED_FEATURE_COLS=58**（`tests/test_production_invariants.py` 鎖定）。
...
> **生產 CLI 指令（2026-07-03 起，preset 單一真相源）**：
> ```bash
...
> ```
> `--production-baseline` 一次套用：topn 30、seasonal-filter、no-stoploss、
> market-filter-tiers、min-pos 2、liq-weighted、pruned-features、**min-avg-turnover 0.5**
> （內容由 `tests/test_production_invariants.py` 鎖定，變更生產配置須同步改測試+本文件）。
> `BACKTEST_ADJ_PRICE_PARQUET` 使 P&L 用還原價（含息），與 label/特徵同口徑——不設會退回
> raw close P&L（混合口徑，數字不可與基準比較）。
> 注意：`topn` 預設 30；若 .env 顯式設了 `TOPN=20` 需手動移除或改為 30。
...
> **基準 v2（2026-07-03 健檢修復後全量重建，兩臂同凍結快照，2016-07-26 ~ 2026-06-04，120 期）**：
>
> | 指標 | 主臂（生產口徑，0.5 億門檻） | 歸因臂（無門檻，舊研究口徑） |
> |------|------|------|
> | 累積報酬 | **+431.06%** | +969.61% |
> | 大盤（含息+同門檻） | +110.29% | +100.94% |
> | 超額 | **+320.77%** | +868.67% |
> | Sharpe | **0.647** | 0.854 |
> | MDD | **-37.05%** | -43.41% |
> | Calmar | 0.498 | 0.626 |
> | 年化 | +18.46% | +27.18% |
> | 勝率 / 盈虧比 | 46.3% / 1.54 | 46.7% / 1.70 |
> | 交易次數 | 2962 | 3066 |
>
> 結果 JSON：`backtest_20260703_204141.json`（主臂）/ `backtest_20260703_205404.json`（歸因臂），
> 均含 `data_snapshot`（prices 5,186,575 / features 4,512,065 / labels 4,971,287 列）。
> **兩臂差 = 0.5 億流動性門檻的真實代價**（微型股 alpha 集中，與 Strategy C 流動性實驗結論一致）。
> **headline 以主臂為準**：它描述的才是實際可部署的東西（universe 門檻 + 部署模型 = 回測模型）。
>
> ⚠️ 與 2026-06-24 的 0.99/+1321% 不可直接比：v2 = 誠實特徵修復（OHLC 全還原、factor 缺日 ffill、
> fear_greed 去 lookahead 等）+ 首次套用生產流動性門檻，兩者混合。同快照下「特徵修復」單獨的影響
> ≈ 歸因臂 0.854 vs 舊 0.99（不同快照，僅供參考、不可證偽）。
>
> 逐年報酬（基準 v2 主臂 vs 歸因臂）：
...
>
> ⚠️ 觀察：門檻臂 2023/2024 幾乎無超額（微型股 alpha 被濾掉後，那兩年的超額大半消失）——
> 「可實際執行的 alpha」比先前認知的小很多，這是誠實化的必然結果而非回歸。
>
> **已退役基準**（口徑不誠實或已被修復推翻，僅供歷史對照）：
> 2026-06-24 生產路徑 0.985/+1321%（損壞 OHLC 特徵）；2026-06-22 survivorship 1.06/+1508%
> （同上+無門檻）；2026-05-23 Stage 10.1 1.33/+5116%（buffer 日曆天殘餘洩漏）；
> 2026-03-13 Experiment F 0.49/+205%（固定停損）。
