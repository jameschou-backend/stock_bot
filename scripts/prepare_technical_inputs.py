#!/usr/bin/env python3
"""Independently copy sealed cash-allocation execution evidence; never fetch data."""
...
PARENT = Path('.cache/cash-allocation')
PARENT_INPUT = Path('.cache/cash-allocation-inputs')
DESTINATION = Path('.cache/technical-inputs')
...
def _destination(root, destination):
    ...
    for name in (PARENT, PARENT_INPUT, Path('.cache/exit-research'), Path('.cache/exit-research-inputs'), Path('.cache/million-replay'),
...
def verify(destination=None, *, root=ROOT):
    ...
    if (data.get('schema') != 1 or data.get('kind') != 'technical-inputs'
            or data.get('preparation_code_sha256') != _sha(Path(__file__))):
        raise InputChanged('Unsupported or changed technical input preparation')
...
def prepare(*, root=ROOT, destination=None):
    ...
    try:
        ...
        data = dict(schema=1, kind='technical-inputs',
