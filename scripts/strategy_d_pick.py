...
Strategy C 的升級版：
- label horizon 5d（vs C 的 10d）：更短線模型，捕捉更快的動能
...
用法：
    python scripts/strategy_d_pick.py                    # 最新有效日期
    python scripts/strategy_d_pick.py --date 2026-03-13  # 指定日期
    python scripts/strategy_d_pick.py --capital 2000000
    python scripts/strategy_d_pick.py --reset-state      # 清空持倉重新開始
"""
...
STATE_FILE  = OUTPUT_DIR / "strategy_d_state.json"

TRAIN_LABEL_HORIZON    = 5     # 5 日 forward return（vs C 的 10d，更短線）
LABEL_BUFFER_DAYS      = 20    # 訓練截止緩衝（避免標籤前向洩漏）
RANK_THRESHOLD         = 0.20  # 維持持倉的最低排名門檻（top 20%）
TOP_ENTRY_N            = 10    # 每日候補選股池
MAX_POSITIONS          = 4     # 最大同時持倉
MAX_HOLD_DAYS          = 30    # 強制出場天數（trailing stop 通常先觸發）
TRAILING_STOP_PCT      = -0.25 # 從持倉最高價回落 25% 即出場（動態鎖利停損）
...
def run_pick(
    ...
    print(f"[Strategy D Pick]  日期：{today}  資金：{capital:,}  (label=5d, trailing stop -25%)")
    ...
    with get_session() as session:
        ...
        # 已修，這條線同步跟上）。price_map／trailing stop／展示欄位仍用 raw close
        # （實際下單價與持倉狀態檔口徑）。factor 表為空時 apply_adj_factors 回退 1.0。
    ...
    # ── 計算 5 日 forward return label（P1-3：用還原價 adj_close，含息口徑）──
    t0 = time.time()
    ...
    for sid, pos in list(positions.items()):
        ...
        close_today = price_map.get(sid, entry_price) or entry_price
        # 更新 peak_price（持倉期間最高收盤價）
        peak_price  = max(pos.get("peak_price", entry_price) or entry_price, close_today)
        pos["peak_price"] = peak_price

        exit_reason = None
        if days >= MAX_HOLD_DAYS:
            exit_reason = "Max Hold Days"
        elif entry_price > 0 and peak_price > 0 and close_today / peak_price - 1 <= TRAILING_STOP_PCT:
            exit_reason = "Trailing Stop"
        elif sid not in above_threshold:
            exit_reason = "Rank Drop"

        if exit_reason:
            sell_list.append({**pos, "stock_id": sid, "exit_reason": exit_reason})
            exits_done.add(sid)
        else:
    ...
    for _, row in candidates.iterrows():
        ...
        buy_list.append({
            ...
            "entry_price": price_map.get(sid, 0.0),
            "peak_price":  price_map.get(sid, 0.0),
            "days_held": 0,
        })
    ...
    for p in hold_list:
        ...
        new_positions[sid] = {
            ...
            "peak_price":  p.get("peak_price", p.get("entry_price", 0.0)),
            "days_held":   p.get("days_held", 0),
        }
    for p in buy_list:
    ...
    out_path = OUTPUT_DIR / f"strategy_d_{today}.json"
...
def _print_result(r: Dict) -> None:
    ...
    print()
    print("=" * 60)
    print(f"  Strategy D 每日選股  {d}  (label=5d + trailing stop -25%)")
    print("=" * 60)
    ...
    out_path = ROOT / "artifacts" / "daily_signal" / f"strategy_d_{r['date']}.json"
