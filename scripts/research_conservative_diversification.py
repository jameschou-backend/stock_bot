#!/usr/bin/env python3
"""Fixed position-count experiment with immutable/offline controls."""
...
OUTPUT = ROOT / '.cache/conservative-diversification-20260924'
SPEC = ROOT / 'docs/prereg_conservative_diversification_20260924.md'
...
def inventory():
    ...
    for path in (SPEC, Path(__file__), ROOT/'skills/conservative_diversification.py', parent.OUTPUT/'manifest.json'):
...
def run_case(data, config, cache, budget, prepare):
    ...
    if config['benchmark']:
        ...
        engine = ConservativeDiversification(*args, **kw, position_count=config['position_count'],
...
def run(output=OUTPUT, prepare=False, offline=False):
    ...
    for stress in ('control','combined'):
        for valid in (3,5,0):
            ...
            config = dict(stress=stress, position_count=valid, benchmark=valid==0)
...
if __name__ == '__main__':
    ...
    a=p.parse_args()
    with file_lock(ROOT/'.cache/conservative-diversification.lock',timeout=0):
