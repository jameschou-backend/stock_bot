...
from skills.pyramid_cache_audit import audit_cache_reuse
import json
from scripts import research_pyramid_cash as study
...
from skills.pyramid_cash_audit import original_entry_view
...
def publish(left,right,proof_path,publication_path):
    ...
    refs=file_identities([Path(__file__),ROOT/'skills/support_risk_source_audit.py',
                         ROOT/'skills/pyramid_cache_audit.py',ROOT/'skills/pyramid_cash_audit.py'],ROOT)
    proof=study.verify(left,right,basic)
    bindings={};cache_audits={}
    with offline_only():
        data,_,_=study.parent.parent.load_data(study.parent.load_selector())
        for folder in (left,right):
            cache_audits[str(folder.relative_to(ROOT))]=audit_cache_reuse(
                read(folder/'report.json'),read(study.PARENT),read(folder/'trials.json'),study.ARMS,
                lambda descriptor:json.loads(verified_bytes(descriptor,ROOT,'.json')))
            bindings[str(folder.relative_to(ROOT))]={}
            for name,row in read(folder/'report.json')['cases'].items():
                value=read(ROOT/row['result']['path'])
                checked=audit_candidate_binding(dict(value,account=original_entry_view(value['account'])),data.entries,data.days)
                if name.startswith('pyramid_') and not value['audit'].get('addition_opportunities_rebuilt'):
                    raise ValueError('Addition signals lack their full journal and source audit')
                bindings[str(folder.relative_to(ROOT))][name]=checked
    if file_identities([ROOT/p for p in refs],ROOT)!=refs:
        raise ValueError('Source-binding audit code changed')
    if (proof['newly_executed_cases']!=sum(x['newly_executed_cases'] for x in cache_audits.values())
            or proof['cache_reuses']!=sum(x['cache_hits'] for x in cache_audits.values())):
        raise ValueError('Reproduction proof overstates actual new executions')
    proof.update(source_binding=bindings,source_binding_code_sha256=refs,cache_audits=cache_audits,
    ...
    value=dict(report,schema='pyramid_cash_publication_v1',adopted=False,strict_data_ready=False,
        ...
        reproduction_trial_count=proof['newly_executed_cases'],
        reproduction_cache_reuses=proof['cache_reuses'],
