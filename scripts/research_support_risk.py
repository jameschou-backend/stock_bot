#!/usr/bin/env python3
"""Reproduce four preregistered support and planned-risk accounts offline."""
...
from skills.support_risk_replay import SupportRiskReplay, ARMS
...
OUTPUT=ROOT/'.cache/support-risk-20260927'
CODE=[Path(__file__),ROOT/'skills/support_risk_replay.py',ROOT/'skills/support_risk_audit.py',
      ROOT/'skills/technical_signals.py',ROOT/'skills/trial_registry.py',
      ROOT/'scripts/research_volatility_budget.py',ROOT/'skills/volatility_budget_replay.py',
      ROOT/'skills/exit_corporate_completion.py',ROOT/DOCUMENT,
      ROOT/'docs/prereg_support_risk_current_20260927.md']


def case(data,inputs,identity,additions,mask,mode,technical):
    config=dict(factor_mask=mask,factors=flags(mask),technical_mode=mode,
    ...
    engine=SupportRiskReplay(data.quotes,data.companies,data.days,data.entries,feeds,corp,
        start=data.start,end=data.end,identity_report=identity,factor_mask=mask,technical_mode=mode,
        technical_signals=technical,exit_signals=data.features,action_dates=list(zip(data.events.stock_id,data.events.event_date)))
    try:
        account=engine.run()
...
def run(output):
    ...
    if output.exists() or not output.is_relative_to(OUTPUT) or output==OUTPUT:
        raise ValueError('Choose a new immutable technical study directory')
    ...
    refs.update(primary)
    if file_identities([ROOT/p for p in refs],ROOT)!=refs:raise ValueError('Source changed before study')
    ...
    with offline_only():
        ...
        for arm in ARMS:
            for mask in range(8):
                ...
                try:
                    value=case(data,inputs,identity,additions,mask,arm,technical)
                    ...
                    record=dict(timestamp=datetime.now(timezone.utc).isoformat(),source='support_risk_current_20260927',
                        command=' '.join(sys.argv),case=name,technical_mode=arm,factor_mask=mask,
    ...
    report=dict(schema='support_risk_research_v1',start=data.start,end=data.end,initial_cash=1_000_000,
...
def verify(left,right,output):
    ...
    proof=dict(schema='support_risk_reproduction_v1',passed=True,compared_cases=32,
